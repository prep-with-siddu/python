# 1.8 Date & Time (Backend Essential) — Notes
...
| # | Video | Creator | Lang |
|---|-------|---------|------|
| 1 | [Datetime Module — Dates, Times, Timedeltas, Timezones](https://www.youtube.com/watch?v=eirjjyP2qcQ) | Corey Schafer | 🇬🇧 |
...
> ⚠️ `timedelta` has no `months` or `years` parameter! Use `dateutil.relativedelta` for those.
...
naive = datetime.now()
print(naive.tzinfo)    # None — DANGEROUS in backends!
...
# ⚠️ Cannot compare naive and aware datetimes!
# naive < utc_aware   # ❌ TypeError!
...
# ✅ Correct way with pytz
aware = ist.localize(naive)
print(aware)    # 2026-02-28 14:30:00+05:30

# ❌ WRONG way (can cause subtle bugs!)
...
> 💡 For new projects, prefer `zoneinfo` (Python 3.9+) over `pytz`. No extra install needed!
...
# ---- ISO 8601 — the standard for APIs and databases ----
...
# ---- Common ISO formats ----
# "2026-02-28"                    — Date only
# "2026-02-28T14:30:00"          — Local (naive)
# "2026-02-28T14:30:00+05:30"    — IST
# "2026-02-28T09:00:00+00:00"    — UTC
# "2026-02-28T09:00:00Z"         — UTC (Z = Zulu = UTC)
```
...
# ⚠️ ALWAYS specify timezone when converting!
# datetime.fromtimestamp(ts)           # ❌ Returns local time (ambiguous)
# datetime.fromtimestamp(ts, tz=utc)   # ✅ Returns UTC
...
# ---- relativedelta — months and years arithmetic ----
from datetime import datetime
...
# → Store as 9:00 AM UTC in database
# → Display as 2:30 PM IST to Indian user
# → Display as 9:00 AM GMT to UK user
# → Display as 4:00 AM EST to US East Coast user

# ---- Why UTC? ----
# ✅ No ambiguity (one source of truth)
# ✅ Daylight saving time doesn't affect stored data
# ✅ Easy to convert to ANY timezone
# ✅ Sorting/comparing works correctly
...
✅ Use zoneinfo/pytz for conversions
✅ Use dateutil for flexible parsing
✅ Use Decimal for time-based billing
❌ Never use naive datetimes in production
❌ Never store in local timezone
❌ Never hardcode timezone offsets (+5:30)
```
...
# Format
dt.strftime("%Y-%m-%d %H:%M:%S")    # → string
datetime.strptime("2026-02-28", "%Y-%m-%d")  # string → datetime
...
# Timestamp
ts = dt.timestamp()                  # datetime → unix
dt = datetime.fromtimestamp(ts, tz=timezone.utc)  # unix → datetime
...
> 📝 **Phase 1 Complete!** Next: Phase 2 — Data Structures
