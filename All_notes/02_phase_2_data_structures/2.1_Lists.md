...
| 3 | [Sorting Lists, Tuples, and Objects](https://www.youtube.com/watch?v=D3JvDWO-BY4) | Corey Schafer | 🇬🇧 |
| 4 | [Python Tutorial For Beginners In Hindi (With Notes) 🔥](https://www.youtube.com/watch?v=gfDE2a7MKjA) | CodeWithHarry | 🇮🇳 |
...
# Basic slicing — start is INCLUDED, stop is EXCLUDED
...
> 💡 **Slicing creates a NEW list** (shallow copy). `nums[2:5]` doesn't modify the original.
...
fruits.extend(["date", "fig"]) # Add MULTIPLE to end → [..., "date", "fig"]
fruits.insert(1, "avocado")    # Insert at INDEX 1 → ["apple", "avocado", "banana", ...]

# append vs extend
fruits.append([1, 2])         # Adds LIST AS ONE ELEMENT → [..., [1, 2]]
fruits.extend([1, 2])         # Adds EACH ELEMENT → [..., 1, 2]

# ---- Removing Elements ----
fruits = ["apple", "banana", "cherry", "banana", "date"]

fruits.remove("banana")       # Remove FIRST occurrence → ["apple", "cherry", "banana", "date"]
# fruits.remove("mango")     # ValueError if not found!

popped = fruits.pop()         # Remove & return LAST → "date"
popped = fruits.pop(1)        # Remove & return at INDEX 1 → "cherry"
...
nums.sort(reverse=True)          # In-place, descending → [9, 6, 5, 4, 3, 2, 1, 1]
nums.reverse()                   # Reverse in-place → [1, 1, 2, 3, 4, 5, 6, 9]
...
| `pop(i)` | Remove & return at index i | The removed item | ✅ Yes |
| `clear()` | Remove all | `None` | ✅ Yes |
| `index(x)` | Find position of x | Index (int) | ❌ No |
...
> 🚨 **Common mistake**: `sort()` returns `None`, not a sorted list! Don't do `x = mylist.sort()`.
...
> 💡 **Rule**: If the comprehension gets too complex (more than ~80 chars), use a regular `for` loop instead. Readability > cleverness.
...
> 💡 Nested comprehensions can be hard to read. If it needs >2 levels of nesting, use regular loops.

---

## 5. `sorted()` vs `.sort()` — Key Parameter, Lambda Sorting
...
> 💡 **Backend tip**: `sorted()` with `key` is how you implement custom ordering in API responses when you can't do it at the database level.
...
### The Problem
```python
# Simple assignment is NOT a copy — both point to the SAME object!
a = [1, 2, 3]
...
### Shallow Copy — Copies the outer list, NOT nested objects
```python
import copy
...
clone = simple.copy()
clone.append(4)
print(simple)    # [1, 2, 3]  ✅ original unchanged
print(clone)     # [1, 2, 3, 4]
...
shallow[2].append(5)
print(original)    # [1, 2, [3, 4, 5]]  😱 nested list was shared!
...
original → [1, 2, ●]           original → [1, 2, ●]
                  ↓                              ↓
shallow  → [1, 2, ●]           deep     → [1, 2, ●]
                  ↓                              ↓
            [3, 4]  ← SHARED!            [3, 4]   [3, 4]  ← INDEPENDENT!
```

| | Shallow Copy | Deep Copy |
|--|-------------|-----------|
| Copies outer list | ✅ Yes | ✅ Yes |
| Copies nested objects | ❌ No (shared) | ✅ Yes (independent) |
| Speed | Fast | Slower |
| Use when | Flat lists, simple data | Nested/complex structures |

> 🚨 **Interview classic**: "What's the difference between shallow copy and deep copy?" — Know this cold.
...
```python
# Use append() and pop() — both O(1) at the end
...
```python
# ❌ Using list as queue — pop(0) is O(n)! Slow!
queue = []
queue.append("a")      # Enqueue
queue.append("b")
queue.append("c")
first = queue.pop(0)   # Dequeue → O(n) because all elements shift!
print(first)            # 'a'

# ✅ Use collections.deque instead — O(1) for both ends
from collections import deque
queue = deque()
queue.append("a")       # Enqueue (right)
queue.append("b")
queue.append("c")
first = queue.popleft() # Dequeue (left) → O(1)!
print(first)             # 'a'
```
...
> 💡 **Always use `deque`** for queues. Never use `list.pop(0)` in production — it's O(n).
