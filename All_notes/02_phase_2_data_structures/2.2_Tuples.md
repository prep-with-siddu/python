...
print(t[0])        # 10
print(t[-1])       # 50
print(t[1:4])      # (20, 30, 40)  — returns a tuple!
...
t[2].append(5)     # ✅ Works!
print(t)            # (1, 2, [3, 4, 5])
# The tuple itself didn't change — it still points to the same list object
...
```python
# Tuple as dict key — works because immutable & hashable
locations = {
...
# locations = {[28.6139, 77.2090]: "Delhi"}  # ❌ TypeError: unhashable type: 'list'
```

> 💡 **Rule of thumb**: Use tuples for data that shouldn't change (coordinates, RGB colors, database rows, config values).
...
### Unpacking in Loops
```python
# Very common pattern — unpacking in for loops
...
> 💡 **Backend tip**: Tuple unpacking is everywhere — `enumerate()`, `dict.items()`, function returns, database query results. Master it.
...
> 💡 **Modern alternative**: Use `dataclasses` for mutable named data, use `namedtuple` when you want immutable lightweight records.
...
> 💡 **Simple rule**: If the data is a **collection** (items of the same kind) → **list**. If it's a **record** (different fields grouped together) → **tuple**.
...
> 📝 **Next up**: Section 2.3 — Dictionaries
