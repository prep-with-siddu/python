...
| # | Video | Creator | Lang |
|---|-------|---------|------|
| 1 | [Dictionaries — Working with Key-Value Pairs](https://www.youtube.com/watch?v=daefaLgNkw0) | Corey Schafer | 🇬🇧 |
| 2 | [Comprehensions — How They Work](https://www.youtube.com/watch?v=3dt4OGnU5sM) | Corey Schafer | 🇬🇧 |
...
# Square bracket notation — raises KeyError if missing
print(user["name"])       # Sid
# print(user["email"])    # ❌ KeyError: 'email'

# .get() — returns None (or default) if missing — SAFER!
...
> 💡 **Backend rule**: ALWAYS use `.get()` when accessing dict values that might not exist. `KeyError` crashes your server!
...
# user.pop("missing")        # ❌ KeyError
user.pop("missing", None)    # ✅ Returns None (no error)

# popitem() — remove and return last inserted pair
last = user.popitem()
print(last)    # ('active', True)

# setdefault() — get value, but SET it if missing
config = {"host": "localhost"}
port = config.setdefault("port", 8000)    # Key missing → sets it AND returns 8000
host = config.setdefault("host", "0.0.0.0") # Key exists → returns "localhost" (no change!)
...
> 💡 `setdefault()` is super useful for grouping: `groups.setdefault(category, []).append(item)`
...
# Backend example — extract specific fields
users_raw = [
...
```python
# Dict of dicts — very common in APIs!
users = {
    "user_1": {
        "name": "Sid",
        "age": 25,
...
> 💡 **Backend tip**: API responses are deeply nested dicts (JSON). Master nested dict access — it's your daily bread.

---

## 5. `defaultdict` from `collections`

Automatically creates a **default value** for missing keys — no more `KeyError`!

```python
from collections import defaultdict

# Regular dict — KeyError on missing
regular = {}
# regular["fruits"].append("apple")    # ❌ KeyError!

# defaultdict — auto-creates on missing
dd = defaultdict(list)                  # Default = empty list
dd["fruits"].append("apple")            # ✅ Auto-creates [] then appends
dd["fruits"].append("banana")
dd["veggies"].append("carrot")
print(dict(dd))    # {'fruits': ['apple', 'banana'], 'veggies': ['carrot']}

# defaultdict(int) — default is 0 (great for counting!)
...
# defaultdict(set) — default is empty set
...
Maintains **insertion order**. Since Python 3.7+, regular `dict` also maintains order — so `OrderedDict` is mostly for older code or when you need `move_to_end()`.
...
> 💡 Use `OrderedDict` when **order matters for equality** or you need `move_to_end()`. Otherwise, regular `dict` is fine in Python 3.7+.
...
> 💡 `Counter` is a lifesaver in **DSA problems** (frequency counting) and **backend analytics** (counting events, errors, etc.).
...
# ChainMap searches first → last
config = ChainMap(cli_args, user_prefs, defaults)
print(config["color"])    # green     (found in cli_args — first dict)
...
### Backend Use Case — Layered Configuration
```python
...
# Before Python 3.9 — use ** unpacking
...
## 10. Hashability — What Can Be a Dict Key
...
d = {
    ...
    3.14: 3,               # float ✅
    True: 4,               # bool ✅
    (1, 2, 3): 5,          # tuple ✅ (if all elements are hashable)
    None: 6,               # NoneType ✅
    frozenset({1,2}): 7,   # frozenset ✅
}
...
# Tuple with mutable element — also unhashable!
# d = {(1, [2, 3]): "bad"}   # ❌ TypeError: unhashable type: 'list'

# Check if hashable
print(hash("hello"))      # Works — some number
...
> 💡 **Why this matters**: Dict keys and set elements must be hashable. This is why you can't use a list as a dict key, but you can use a tuple.
...
d.get("key", "default")    # Safe — returns default
