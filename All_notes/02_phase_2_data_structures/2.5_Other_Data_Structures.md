# 2.5 Other Data Structures — Notes
...
| # | Video | Creator | Lang |
|---|-------|---------|------|
| 1 | [Itertools Module — Iterator Functions for Efficient Looping](https://www.youtube.com/watch?v=Qu3dThVy6KQ) | Corey Schafer | 🇬🇧 |
| 2 | [Python Full Course for free 🐍](https://www.youtube.com/watch?v=ix9cRaBkVe0) | Bro Code | 🇬🇧 |
...
A **deque** (pronounced "deck") supports fast appends and pops from **both ends** — O(1) vs O(n) for lists!
...
### `maxlen` — Fixed-Size Deque (Sliding Window!)
```python
...
recent.append("page_4")   # deque(['page_2', 'page_3', 'page_4'])  ← page_1 dropped!
...
# List — O(n) per insert at front → O(n²) total
lst = []
start = time.time()
for i in range(n):
...
# Deque — O(1) per appendleft → O(n) total
dq = deque()
start = time.time()
for i in range(n):
...
| Use Case | Why deque? |
|----------|-----------|
| Queue (FIFO) | `append()` + `popleft()` — both O(1) |
...
queue.append("task_2")
queue.append("task_3")
task = queue.popleft()       # Dequeue → "task_1"
print(task)                  # task_1
...
## 2. `heapq` — Heap Queue (Priority Queue)
...
## 3. `array` Module — Typed Arrays

Unlike lists (which store any type), `array` stores elements of **one type** — uses less memory.

```python
from array import array

# Create — must specify type code
...
> 💡 Use `array` for **large collections of same-type numbers** where memory matters. For most cases, use `list`. For heavy math, use **NumPy** instead.

---

## 4. `bisect` Module — Binary Search on Sorted Lists
...
# bisect_left — insert BEFORE existing duplicates
pos = bisect.bisect_left(sorted_list, 30)
print(pos)    # 2 (index where 30 is)

# bisect_right (or just bisect) — insert AFTER existing duplicates
...
### Backend Example — API Status Codes
```python
from enum import Enum
...
update_order(123, OrderStatus.SHIPPED)  # Order 123 → shipped
...
### IntEnum — Enum That Acts Like int
```python
...
### Flag Enum — Bitwise Combinations
```python
...
> 💡 **Backend tip**: Use `str, Enum` for API fields in FastAPI/Pydantic. It auto-validates and generates docs!
...
> 📝 **Phase 2 Complete!** Next: Phase 3 — Functions (already in `03_phase_3_functions/`)
