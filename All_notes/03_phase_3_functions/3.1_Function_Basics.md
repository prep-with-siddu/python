...
| # | Video | Creator | Lang |
|---|-------|---------|------|
| 1 | [Python Tutorial for Beginners 8: Functions](https://www.youtube.com/watch?v=9Os0o3wzS_I) | Corey Schafer | 🇬🇧 |
| 2 | [Args and Kwargs — Intermediate Python](https://www.youtube.com/watch?v=gZB_ENJD34E) | sentdex | 🇬🇧 |
...
| Name in `snake_case` | `def calculate_total():` ✅ not `def CalculateTotal():` ❌ |
...
> 💡 **Naming tip**: Functions should be verbs or verb phrases — `get_user()`, `calculate_tax()`, `send_email()`, not `user()` or `tax()`.
...
### Parameter vs Argument — Don't Confuse!
```python
...
> 💡 **Parameters** = what the function expects. **Arguments** = what you actually give it.
...
> 💡 **Backend tip**: Functions that process data should **always return**, not just `print()`. Printing is for debugging, returning is for building APIs.
...
# ❌ Wrong — default before non-default causes SyntaxError
# def create_user(role="user", name):   # SyntaxError!
```

### ⚠️ DANGER: Mutable Default Arguments (Common Interview Question!)
```python
# ❌ BUG — mutable default (list) is shared across ALL calls!
...
> 🚨 **This is a top Python interview question!** Never use mutable objects (`list`, `dict`, `set`) as default parameter values. Always use `None` and create inside the function.
...
power(3, 2)     # 9  → base=3, exponent=2  (different result! order matters)
```
...
# greet("Sid", "Hey")                       # ❌ TypeError! Must use keyword
...
add(5, 3)           # ✅ 8
# add(a=5, b=3)     # ❌ TypeError! Must be positional
...
> 💡 **Backend tip**: In APIs, forcing keyword-only arguments makes function calls more readable — `create_order(user_id=5, product_id=10)` is clearer than `create_order(5, 10)`.
...
### Real-World Usage — Backend Example
```python
...
> 💡 **When to use**:
> - `*args` → When you don't know how many arguments will be passed (like `print()` itself!)
> - `**kwargs` → When you want to accept any keyword argument (common in decorators, wrappers, configs)
...
#### Google Style (Recommended — most readable)
```python
...
1. **API documentation** — Tools like Swagger auto-generate docs from docstrings
2. **Team collaboration** — Others understand your code without reading implementation
3. **IDE support** — VS Code / PyCharm show docstrings on hover
4. **`help()`** — Built-in way to read documentation

> 💡 **Rule of thumb**: Every function that will be used by other developers (or your future self) should have a docstring. At minimum: what it does, what it takes, what it returns.
...
# Unpacking
func(*list_var)               # Unpack list → positional
func(**dict_var)              # Unpack dict → keyword
...
> 📝 **Next up**: Section 3.2 — Advanced Functions (lambda, closures, decorators, generators, itertools)
