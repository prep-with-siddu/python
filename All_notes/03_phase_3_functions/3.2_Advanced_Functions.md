...
| # | Video | Creator | Lang |
|---|-------|---------|------|
| 1 | [Decorators — Dynamically Alter Functions](https://www.youtube.com/watch?v=FsAPt_9Bf3U) | Corey Schafer | 🇬🇧 |
| 2 | [Closures — How to Use Them and Why](https://www.youtube.com/watch?v=swU3c34d2NQ) | Corey Schafer | 🇬🇧 |
| 3 | [Generators — How to Use Them](https://www.youtube.com/watch?v=bD05uGo_sVI) | Corey Schafer | 🇬🇧 |
| 4 | [Comprehensions — How They Work](https://www.youtube.com/watch?v=3dt4OGnU5sM) | Corey Schafer | 🇬🇧 |
| 5 | [Itertools Module](https://www.youtube.com/watch?v=Qu3dThVy6KQ) | Corey Schafer | 🇬🇧 |
| 6 | [Python Tutorial For Beginners In Hindi (With Notes) 🔥](https://www.youtube.com/watch?v=gfDE2a7MKjA) | CodeWithHarry | 🇮🇳 |
...
> 💡 Functions being first-class is what makes **decorators**, **callbacks**, and **strategy patterns** possible in Python.
...
Python has several built-in higher-order functions: `map()`, `filter()`, `sorted()`, `min()`, `max()` — all accept a function as argument.
...
> 💡 If a function takes another function as input or returns one — it's a higher-order function.
...
# ❌ Bad practice — don't assign lambda to a variable
...
> 💡 **Rule**: Use `lambda` for short, throwaway functions (especially in `sorted()`, `map()`, `filter()`). Use `def` for anything else.
...
# map(function, iterable) → returns map object (lazy)
...
### `reduce()` — Reduce all elements to a single value
```python
...
# 💡 Prefer built-in sum(), max(), min() over reduce for simple cases
print(sum(numbers))    # 15 — much cleaner!
```
...
> 💡 **In modern Python, list comprehensions are preferred** over `map()`/`filter()`. But know both — they appear in interviews and legacy code.
...
hello_func()              # Hello!  — remembers "Hello!" even though outer() finished!

bye_func = outer("Goodbye!")
bye_func()                # Goodbye!  — different closure with different message
```
...
print(closure.__closure__[0].cell_contents)   # 10  — the captured value!
```

> 💡 Closures are the **foundation of decorators**. Understand closures first, and decorators will make perfect sense.
...
outer()
# Inner count: 10
# Outer count: 0  ← outer's count was NOT modified!

# With nonlocal — modifies the enclosing variable
def outer():
    count = 0
    def inner():
...
outer()
# Inner count: 10
# Outer count: 10  ← outer's count WAS modified!
```
...
outer()
print(x)                  # "global"  — global x was NOT touched
```

> 💡 Use `nonlocal` sparingly. It's mainly needed in closures and decorators when you need to maintain state.

---

## 7. Decorators — Creating and Using
...
```python
# Decorator WITHOUT arguments: 2 levels (decorator → wrapper)
# Decorator WITH arguments: 3 levels (decorator_factory → decorator → wrapper)
...
# Execution order:
# 1. italic wraps greet → italic_wrapper
# 2. bold wraps italic_wrapper → bold_wrapper
# Same as: greet = bold(italic(greet))
```

> 💡 **Read decorators bottom-to-top** — the one closest to `def` runs first.
...
> 🚨 **Always use `@wraps(func)`** in every decorator you write. It's a best practice.
...
# Without cache — SLOW (recalculates same values repeatedly)
def fib(n):
    if n <= 1:
        return n
...
# With lru_cache — FAST (caches results)
...
### `@cache` (Python 3.9+) — Simpler, Unbounded
```python
...
print(expensive_computation(5))    # 25 (no "Computing..." — cached!)
```

> 💡 **Backend use case**: Cache database queries, API responses, or expensive calculations. But be careful with memory usage — use `maxsize` to limit cache.
...
# Backend example — pre-configured logger
import logging
...
> 💡 `partial` is great when you need a function with **fewer arguments** than the original — e.g., passing to `map()`, `sorted()`, or callback handlers.
...
# Too deep recursion → RecursionError
```

> 💡 **In backend work**, recursion is used for tree traversals (nested comments, categories), parsing nested JSON, and graph algorithms. But for simple problems, **prefer loops** — they're faster and don't hit the recursion limit.
...
```python
# Not tail recursive — has pending multiplication after recursive call
def factorial(n):
    if n <= 1:
        return 1
    return n * factorial(n - 1)    # After factorial returns, still need to multiply by n

# Tail recursive version — result built up in accumulator
...
> 💡 **Know the concept** for interviews, but in practice use iterative solutions or memoization in Python.

---

## 15. Generator Functions (`yield`)

A **generator** is a function that returns values **one at a time** using `yield`, instead of all at once with `return`. It's **lazy** — computes values on demand.
...
# print(next(squares))  # StopIteration error — exhausted!
...
# Generator — stores only the logic, not the data
...
> 💡 **Generators are crucial for backend** — reading large files, streaming data, processing database results without loading everything into memory.
...
# Generator expression — creates lazy generator
...
> 💡 Use **generator expressions** when you only need to iterate once and don't need the full list stored in memory.
...
> 💡 `yield from` is cleaner than looping and yielding manually. It also properly handles the generator's `return` value and exceptions.
...
# islice — slice a generator (can't use [start:stop] on generators!)
...
| `cycle(iterable)` | Infinite cycling | `cycle("AB")` → A, B, A, B... |
| `repeat(val, n)` | Repeat n times | `repeat("x", 3)` → x, x, x |
| `chain(*iterables)` | Concatenate | `chain([1,2],[3,4])` → 1,2,3,4 |
...
> 💡 `itertools` is heavily used in **DSA problems** and **data processing** in backend. Master `chain`, `groupby`, `product`, `combinations`, and `islice`.
...
### Type Hints Don't Enforce — They're Just Hints!
```python
def add(a: int, b: int) -> int:
    return a + b
...
1. **IDE support** — autocompletion, error detection in VS Code/PyCharm
2. **Pydantic** — FastAPI uses type hints for automatic request validation
3. **Documentation** — anyone reading your code knows what types are expected
4. **`mypy`** — catches type bugs before runtime
5. **Team collaboration** — essential in professional codebases

```python
# FastAPI example — type hints become API validation!
...
> 💡 **Start using type hints NOW.** It's a habit that will make you a better backend developer. FastAPI is entirely built around type hints.
...
> 📝 **Next up**: Phase 4 — Object-Oriented Programming (OOP)
