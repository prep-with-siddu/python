...
| # | Video | Creator | Lang |
|---|-------|---------|------|
| 1 | [OOP Tutorial 4: Inheritance — Creating Subclasses](https://www.youtube.com/watch?v=RSl87lqOXDE) | Corey Schafer | 🇬🇧 |
| 2 | [OOP Tutorial 1: Classes and Instances](https://www.youtube.com/watch?v=ZDa-Z5JzLYM) | Corey Schafer | 🇬🇧 |
...
class User:
    def __init__(self, name, email, password):
        ...
        self._email = email        # Protected — "please don't touch" (convention only)
        self.__password = password # Private — name mangling (harder to access)
...
# ⚠️ Protected — works, but signals "internal use"
...
# 🔓 But you CAN access it via name mangling (don't do this!)
...
class BankAccount:
    def __init__(self, owner, balance):
        self._owner = owner
        self.__balance = balance    # Private — protect from direct modification
...
# ❌ Cannot directly set balance (no bypassing validation!)
...
|--------|------|--------|-----------|
| `name` | Public | Anyone | — |
| `_name` | Protected | Internal/subclass | ❌ Convention only |
| `__name` | Private | Name mangled | ⚠️ Partially (`_Class__name`) |
| `__name__` | Dunder/Magic | Python special methods | — |

> 💡 Python philosophy: **"We're all consenting adults here."** Single underscore `_` is the most common convention for "private".
...
### Backend Example — Base Model Pattern
```python
from datetime import datetime

class BaseModel:
    """Common fields for all models — like Django's abstract model"""
    def __init__(self):
        self.id = None
...
print(dog.breathe())      # Breathing...   (from Animal — 2 levels up!)
```
...
> 💡 Multiple inheritance is powerful but can get messy. Prefer **composition** or **mixins** for cleaner code.
...
# D.__init__
# B.__init__
# C.__init__    ← super() in B calls C (not A!) because MRO is D→B→C→A
# A.__init__
```
...
> 💡 **Interview tip**: Always know MRO. It's the #1 multiple inheritance question. Use `ClassName.mro()` or `ClassName.__mro__` to check.
...
Python doesn't care about the **type** — it cares about the **behavior** (methods/attributes).
...
# Works with ANYTHING that has .speak() — no inheritance required!
```
...
# Function doesn't care WHAT the logger is — just that it has .write()
...
```python
# len() works on any object with __len__ — polymorphism!
print(len("hello"))       # 5  (str)
print(len([1, 2, 3]))     # 3  (list)
print(len({"a": 1}))      # 1  (dict)

# + operator works differently for different types — polymorphism!
print(1 + 2)              # 3        (int addition)
print("a" + "b")          # ab       (string concatenation)
print([1] + [2])          # [1, 2]   (list concatenation)

# for loop works on any iterable — duck typing!
...
> 💡 **Python motto**: "It's easier to ask forgiveness than permission" (EAFP). Try calling the method, catch the exception if it fails.

---

## 5. Abstraction (`abc` Module, `ABC`, `abstractmethod`)

Abstraction = **hide complex details**, show only what's necessary. Abstract classes **define the contract** — subclasses **must implement** the abstract methods.

```python
from abc import ABC, abstractmethod

class PaymentProcessor(ABC):    # Abstract Base Class — CANNOT be instantiated
...
# ❌ Missing refund() — will raise TypeError
...
```python
# Function accepts ANY PaymentProcessor — doesn't care which gateway
def checkout(processor: PaymentProcessor, amount: float):
    result = processor.process_payment(amount)
    print(f"Result: {result}")

# Works with any implementation!
checkout(RazorpayProcessor(), 999)    # Razorpay: Processing ₹999
checkout(StripeProcessor(), 999)      # Stripe: Processing ₹999
```
...
| Scenario | Use |
|----------|-----|
| Define a contract/interface | ✅ ABC |
| Share common code + enforce methods | ✅ ABC |
| Just need shared behavior (no enforcement) | Regular inheritance |
| Multiple unrelated behaviors | Duck typing / Protocol |

> 💡 **Backend pattern**: Abstract base classes are perfect for defining **interfaces** — PaymentProcessor, NotificationSender, StorageBackend, etc. Swap implementations without changing the calling code.
...
class MyClass:
    ...
    __private = "mangled"   # Private (name mangling → _MyClass__private)
...
> 📝 **Next up**: Section 4.3 — OOP Advanced (classmethod, staticmethod, properties, dunder methods, dataclasses, SOLID)
