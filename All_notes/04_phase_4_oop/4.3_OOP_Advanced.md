...
| # | Video | Creator | Lang |
|---|-------|---------|------|
| 1 | [OOP Tutorial 3: classmethods and staticmethods](https://www.youtube.com/watch?v=rq8cL2XMM5M) | Corey Schafer | 🇬🇧 |
| 2 | [OOP Tutorial 5: Special (Magic/Dunder) Methods](https://www.youtube.com/watch?v=3ohzBxoFHAY) | Corey Schafer | 🇬🇧 |
| 3 | [OOP Tutorial 6: Property Decorators — Getters, Setters, Deleters](https://www.youtube.com/watch?v=jCzT9XFZ5bw) | Corey Schafer | 🇬🇧 |
| 4 | [Metaclasses & How Classes Really Work](https://www.youtube.com/watch?v=NAQEj-c2CI8) | Tech With Tim | 🇬🇧 |
...
class User:
    ...
    # Instance method — gets self (the object)
    def greet(self):
    ...
    def from_string(cls, user_string):
        ...
        return cls(name, email)    # cls() = User() — works with subclasses too!
...
### Why `cls` Instead of `User`? — Inheritance-Safe
```python
class User:
    @classmethod
    def create(cls, name):
...
# cls = AdminUser here (not User!) — returns AdminUser instance
admin = AdminUser.create("Sid")
print(type(admin))    # <class 'AdminUser'>  ✅
print(admin.role)     # admin
```

### `@staticmethod` — No `self` or `cls`, Just a Regular Function Inside a Class
```python
class MathUtils:
    @staticmethod
    def add(a, b):
...
| Access class? | ✅ Via `self.__class__` | ✅ Via `cls` | ❌ No |
| Use case | Object behavior | Factory methods, class-level ops | Utility/helper functions |

> 💡 **Rule of thumb**: If the method doesn't use `self` → make it `@staticmethod`. If it needs the class but not the instance → `@classmethod`.

---

## 2. Properties (`@property`, `@setter`, `@deleter`)

Properties let you **use methods like attributes** — add validation/logic without changing the API.
...
class User:
    ...
    @property
    def age(self):
        """Getter — called when you READ user.age"""
        return self._age
    
    @age.setter
    def age(self, value):
        """Setter — called when you WRITE user.age = x"""
    ...
    @age.deleter
    def age(self):
        """Deleter — called when you do del user.age"""
        print("Deleting age...")
        self._age = None
...
# r.area = 50        # ❌ AttributeError: can't set attribute (read-only!)
```

> 💡 **Backend tip**: Properties are used everywhere in Django models and Pydantic models. They let you add logic without breaking existing code.
...
class Vector:
    ...
    def __hash__(self):                  # hash(v) — needed for sets/dict keys
    ...
    # ---- Callable ----
    def __call__(self):                  # v() — treat object like a function
...
class Money:
    ...
    def __repr__(self):
        return f"₹{self.amount}" if self.currency == "INR" else f"{self.currency} {self.amount}"
...
> 💡 Descriptors are the mechanism behind `@property`. You rarely write them directly, but understanding them helps you understand Python's internals.
...
user = User("Sid", 25)
print(user)    # User(name='Sid', age=25)  ← auto-generated!
```

> 💡 **Practical rule**: You'll rarely write metaclasses. But know they exist — Django ORM, SQLAlchemy, and Pydantic all use them internally.

---

## 7. `__slots__` for Memory Optimization

By default, Python stores instance attributes in a `__dict__` (a dict per object). `__slots__` replaces it with a fixed structure — saves memory.
...
# u2 has no __dict__!
# print(u2.__dict__)                 # ❌ AttributeError

# ---- Slots prevent dynamic attributes ----
u1.email = "sid@mail.com"    # ✅ Works (dict-based)
# u2.email = "sid@mail.com"  # ❌ AttributeError: 'UserWithSlots' has no attribute 'email'

# ---- When it matters ----
# Creating 1,000,000 objects → __slots__ can save ~40-50% memory
```
...
Dataclasses auto-generate `__init__`, `__repr__`, `__eq__`, and more — less boilerplate!
...
print(hash(p))  # ✅ Can be used as dict key / set element
```
...
> 💡 **Backend essential**: Dataclasses are used in FastAPI request/response models, configuration objects, DTOs (Data Transfer Objects).
...
| Converters | ❌ No | ✅ Yes |
| Slots | Manual | ✅ `@attr.s(slots=True)` |
| Recommendation | Use for simple cases | Use for complex validation |

> 💡 For backend: Use **Pydantic** instead of attrs (it does validation + serialization). Dataclasses for simple DTOs.
...
> 💡 **Mixin rules**: (1) Mixins should be small, single-purpose. (2) They should NOT have `__init__`. (3) Name them `XxxMixin`.
...
class Car:
    def __init__(self, brand, horsepower):
        ...
        self.gps = GPS()                    # Composition — HAS-A
...
### Backend Example — Composition Over Inheritance
```python
# ❌ Bad: Inheritance for code reuse → tight coupling
class UserRepository(DatabaseConnection, CacheManager, Logger):
    pass    # Inherits from 3 things — messy!

# ✅ Good: Composition → loose coupling, flexible
...
> 💡 **Golden rule**: "Favor composition over inheritance." — Gang of Four Design Patterns
...
# ✅ New payment methods = new class, no modification of existing code
...
# Adding new payment → just add a new class!
class CryptoPayment(PaymentProcessor):
    def pay(self, amount):
        print(f"Paid ₹{amount} via Crypto")

def checkout(processor: PaymentProcessor, amount):
    processor.pay(amount)    # Works with ANY processor — past, present, future
```
...
# @classmethod — alternative constructors, class-level ops
# @staticmethod — utility functions (no self/cls)
# @property — attribute-like access with getter/setter logic
# Dunder methods — customize built-in behavior (__add__, __eq__, __len__, etc.)
# __slots__ — memory optimization (no __dict__)
# @dataclass — auto-generate __init__, __repr__, __eq__
# Mixins — small, focused classes mixed via multiple inheritance
# Composition — "HAS-A" → prefer over inheritance
# SOLID — S: one job, O: extend not modify, L: substitutable,
#          I: small interfaces, D: depend on abstractions
```

---

> 📝 **Phase 4 Complete!** Next: Phase 5 — Error Handling & File Operations
