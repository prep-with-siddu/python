...
| # | Video | Creator | Lang |
|---|-------|---------|------|
| 1 | [Using Try/Except Blocks for Error Handling](https://www.youtube.com/watch?v=NIWwJbo-9_8) | Corey Schafer | 🇬🇧 |
| 2 | [Logging Basics — Logging to Files, Levels, Formatting](https://www.youtube.com/watch?v=-ARI4Cz-awo) | Corey Schafer | 🇬🇧 |
...
try:
    ...
    # ALWAYS runs — exception or not, even if you return!
    print("Cleanup done.")
...
try:
    ...
    print(f"Result: {result}")          # ← Skipped (exception happened)
finally:
...
> 💡 **Backend rule**: Use `else` for code that should only run on success. Use `finally` for cleanup (closing connections, files, etc.).
...
```python
# ❌ BAD — bare except catches EVERYTHING (even KeyboardInterrupt, SystemExit!)
try:
    x = int("abc")
except:
...
try:
    open("/nonexistent/file.txt")
except Exception:             # Catches EVERYTHING — specific blocks never reached!
    print("Something went wrong!")
except FileNotFoundError:     # ⚠️ NEVER reached! Dead code!
    print("File not found!")
```
...
> 💡 **Rule**: Always catch `Exception` or more specific. Never catch `BaseException`.
...
> 💡 **Backend pattern**: Create a hierarchy of custom exceptions → catch them in middleware → return proper HTTP error responses.
...
try:
    try:
        ...
        # New exception during handling → implicit chain
        d = {}
...
> 💡 **Backend tip**: Use `raise ... from e` to wrap low-level errors (database, network) into meaningful app-level errors. Keeps the cause chain for debugging.

---

## 8. Context Managers (`with` Statement)

Context managers auto-handle **setup** and **cleanup** — guaranteed, even if exceptions occur.
...
f = open("test.txt", "w")
f.write("hello")
# If error here → file never closed!
f.close()
...
def __exit__(self, exc_type, exc_val, exc_tb):
    ...
    # exc_val   → Exception instance (e.g., ValueError("bad"))  or None
...
The **easier** way to create context managers — using a generator function!
...
### Real-World Backend Example — Transaction Manager
```python
from contextlib import contextmanager

@contextmanager
def transaction(connection):
    """Manages database transaction — commits on success, rolls back on error"""
...
#     # If error → auto rollback. If success → auto commit.
```
...
# closing — auto-call .close() on exit
...
> 💡 **Backend rule**: ALWAYS use `logging`, NEVER use `print()` in production. Use `logger.exception()` to capture tracebacks.
...
> 📝 **Next up**: Section 5.2 — File Handling
