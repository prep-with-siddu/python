...
| # | Video | Creator | Lang |
|---|-------|---------|------|
| 1 | [File Objects — Reading and Writing to Files](https://www.youtube.com/watch?v=Uh2ebFW8OYM) | Corey Schafer | 🇬🇧 |
| 2 | [OS Module — Use Underlying OS Functionality](https://www.youtube.com/watch?v=tJxcKyFMTGo) | Corey Schafer | 🇬🇧 |
| 3 | [Working with JSON Data using the `json` Module](https://www.youtube.com/watch?v=9N6a-VLBa2I) | Corey Schafer | 🇬🇧 |
...
```python
# ---- open() and read() — entire file at once ----
...
# ---- Iterating line by line (BEST for large files — memory efficient!) ----
with open("sample.txt", "r") as f:
...
> 💡 **Backend rule**: For large files, NEVER use `read()` or `readlines()` — they load everything into memory. Iterate line by line.
...
# Exclusive create — safe for creating new files
try:
...
```python
# ❌ Without `with` — risky! File might not close on error
f = open("data.txt", "r")
try:
    content = f.read()
...
# ✅ With `with` — automatic close, guaranteed!
with open("data.txt", "r") as f:
    content = f.read()
...
> 💡 **Rule**: ALWAYS use `with` for file operations. No exceptions (pun intended).
...
> 💡 **Backend tip**: For large CSV processing, use `pandas`. For simple I/O, `csv` module is fine.
...
# ✅ Or use default= parameter (simpler)
...
> 💡 **Backend essential**: JSON is the language of APIs. You'll use `json` module daily.
...
with open("config.yaml", "r") as f:
    loaded = yaml.safe_load(f)    # ✅ Always use safe_load (not yaml.load!)
    print(loaded["database"]["host"])    # localhost

# ⚠️ NEVER use yaml.load() without Loader — security risk!
# yaml.load(f)                  # ❌ Dangerous — can execute arbitrary code
# yaml.safe_load(f)             # ✅ Safe
# yaml.load(f, Loader=yaml.SafeLoader)  # ✅ Also safe
```

> 💡 Use YAML for config files (Docker Compose, Kubernetes, CI/CD). Use JSON for APIs.
...
> 💡 XML is less common in modern APIs (JSON won), but you'll encounter it in SOAP APIs, RSS feeds, and legacy systems.
...
> 💡 **Backend essential**: EVERY backend project uses `.env` for secrets. NEVER hardcode passwords, API keys, or database URLs.

---

## 10. `pathlib` Module (Modern Path Handling)

`pathlib` is the **modern** way to handle paths — object-oriented, cross-platform.
...
> 💡 **Best practice**: Use `pathlib` for all new code. It's cleaner and more Pythonic.
...
# ---- Delete directory (recursive — DANGEROUS!) ----
...
# Download → process → upload (don't pollute disk)
...
> 💡 **Backend tip**: Use `tempfile` for processing uploads, generating reports, and any file that doesn't need to persist.
...
> 📝 **Phase 5 Complete!** Next: Phase 6 — Modules, Packages & Project Structure
