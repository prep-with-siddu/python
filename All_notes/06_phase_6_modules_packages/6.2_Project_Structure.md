...
| # | Video | Creator | Lang |
|---|-------|---------|------|
| 1 | [virtualenv and why you should use virtual environments](https://www.youtube.com/watch?v=N5vscPTWKOk) | Corey Schafer | 🇬🇧 |
| 2 | [Import Modules and Exploring The Standard Library](https://www.youtube.com/watch?v=CqvZ3vGoGs0) | Corey Schafer | 🇬🇧 |
...
```
my_project/
├── my_project/           ← Source code (same name as project)
│   ├── __init__.py
│   └── main.py
├── tests/
│   ├── __init__.py
...
└── requirements/
    ...
    ├── dev.txt                 ← Dev tools (pytest, black, etc.)
...
> 💡 **Src layout** prevents a common bug: accidentally importing from source instead of installed package during testing.
...
```python
# setup.py — the OLD way to configure Python packages
...
### `pyproject.toml` (MODERN — Use This!)
```toml
# pyproject.toml — the MODERN standard (PEP 518, PEP 621)
...
| Recommended | For old projects | **For all new projects** ✅ |

> 💡 **Rule**: Always use `pyproject.toml` for new projects. `setup.py` is legacy.
...
### `poetry.lock`
```
# poetry.lock — ALWAYS commit this file!
...
| Community | Growing fast ✅ | Declining |
...
> 💡 Use [gitignore.io](https://gitignore.io) to generate `.gitignore` for any stack. Search "Python" and it gives you a comprehensive one.

---

## 7. `Makefile` for Common Commands

A `Makefile` is a shortcut system — run complex commands with simple names.

```makefile
# Makefile — put this at project root
...
> 💡 **Makefile** makes onboarding easy — new devs just run `make install && make run`.

---

## 8. `pre-commit` Hooks

Automatically run checks **before every git commit** — catches errors early.
...
```yaml
# .pre-commit-config.yaml — put at project root
...
# → black runs → isort runs → ruff runs → mypy runs → commit (if all pass)
...
git commit --no-verify -m "hotfix: skip hooks"    # ❌ Avoid this!
```
...
# Speed comparison:
# pip install django     → ~10 seconds
# uv pip install django  → ~0.5 seconds  (20x faster!)
```
...
|---------|-----|--------|--------|
| Speed | Slow | Medium | **20-100x faster** ✅ |
| Lock file | No | Yes | Yes |
| Venv management | Manual | Built-in | Built-in |
| Python installer | No | No | **Yes** ✅ |
| Resolver | Basic | Good | **Best** ✅ |
...
> 💡 **2026 recommendation**: Use `uv` for new projects. It's the future of Python packaging.
...
```
company-backend/                    ← Single git repo
├── services/
│   ├── auth-service/
│   │   ├── src/
│   │   ├── tests/
...
|---------|----------|------------|
| Code sharing | Easy (shared folder) ✅ | Hard (publish packages) |
| CI/CD | Complex (selective builds) | Simple per service ✅ |
| Code review | See all changes together ✅ | Separate PRs |
| Dependencies | Shared versions ✅ | Independent |
| Team autonomy | Lower | Higher ✅ |
| Repo size | Large | Small ✅ |
| Used by | Google, Meta, Uber | Netflix, Amazon |
| Good for | Small-medium teams ✅ | Large teams ✅ |

> 💡 **Startup advice**: Start with a monorepo. Split into multi-repo only when team grows and services are truly independent.
...
1.0.1 → 1.1.0    # MINOR: New feature added (safe to upgrade)
1.1.0 → 2.0.0    # MAJOR: Breaking change (READ changelog first!)
...
~=1.4.0     >=1.4.0, <1.5.0  (compatible release — patch updates only)
^1.4.0      >=1.4.0, <2.0.0  (poetry — minor updates OK)
...
| Change response format | MAJOR (X.0.0) | Rename `user_name` → `username` |
...
> 💡 **API rule**: If existing clients will break → it's a MAJOR version change.
...
uv add X                    # Add dependency (uv — fastest!)
...
^1.4.0 → >=1.4.0, <2.0.0   # Poetry compatible
~=1.4.0 → >=1.4.0, <1.5.0  # pip compatible
```

---

> 📝 **Phase 6 Complete!** Next: Phase 7 — Advanced Python (Iterators, Generators, Concurrency, Memory, Metaprogramming)
