# 7.2 Decorators (Deep Dive) — Notes
...
A decorator **wraps** a function to add behavior — without changing the original function.
...
db2 = Database("postgres://localhost/other")   # No print — same instance!
...
# What happens:
# repeat(times=3) → returns decorator function
# @decorator → decorates say_hi → returns wrapper
# say_hi("Sid") → calls wrapper("Sid")
```
...
create_user("Sid", 25)        # ✅ Works
create_user("Sid", "twenty")  # ❌ TypeError: Argument 'age' must be int, got str
```
...
@smart_decorator          # No parentheses — works!
def func_a():
    pass

@smart_decorator(prefix="DEBUG")    # With arguments — also works!
def func_b():
    pass
...
### Why It Matters
```python
# 1. Debugging — stack traces show correct function name
# 2. Documentation — help(greet) shows correct docstring
# 3. Introspection — tools like pytest, Sphinx, Flask routing depend on __name__
# 4. Access original — greet.__wrapped__ gives the undecorated function
...
> 📝 **Next up**: 7.3 Concurrency & Parallelism — GIL, threading, multiprocessing, asyncio
