...
# When refcount reaches 0 → object is deallocated instantly
...
# Both objects have refcount > 0 → CAN'T be freed by refcounting!
...
# See what GC tracks
print(gc.get_count())    # (123, 5, 1) — objects in each generation
```

### Memory Lifecycle
```
Object Created → Reference Counting Tracks It
    │
    ├── refcount drops to 0 → IMMEDIATELY freed (most objects)
    │
    └── Circular reference? → Garbage Collector finds and frees it
```
...
print(referents)    # [{'data': [1, 2, 3]}] — its __dict__
...
# ⚠️ WARNING: getsizeof only shows SHALLOW size!
...
A weak reference **does NOT increase the reference count** — object can be garbage collected.
...
ref = obj        # Strong reference → refcount = 2
del obj          # refcount = 1 → NOT destroyed!
print(ref)       # ExpensiveObject(strong) — still alive!
del ref          # refcount = 0 → "strong destroyed!"

# ---- Weak reference (doesn't keep alive) ----
obj = ExpensiveObject("weak")
weak_ref = weakref.ref(obj)    # Weak reference → refcount stays 1

print(weak_ref())     # ExpensiveObject(weak) — access via ()
del obj               # refcount = 0 → "weak destroyed!"
...
del u1, u2           # No strong references → automatically removed from cache!
...
By default, Python stores instance attributes in a `__dict__` (dictionary). `__slots__` replaces that with a fixed tuple — saves memory.
...
# Normal:  ~280 bytes × 1M = ~280MB
# Slots:   ~56 bytes × 1M  = ~56MB  (5x less memory!)
```
...
# u.email = "sid@example.com"    → AttributeError!

# ❌ No __dict__
# print(u.__dict__)              → AttributeError!

# ✅ Can still have methods, properties, class variables
class Product:
...
# ✅ Slots with inheritance (add parent slots)
class Admin(User):
...
class MyClass:
    __slots__ = ("attr1", "attr2")   # No __dict__ → 5x less memory
```

---

> 📝 **Next up**: 7.5 Metaprogramming — Metaclasses, `type()`, descriptors, `inspect`
