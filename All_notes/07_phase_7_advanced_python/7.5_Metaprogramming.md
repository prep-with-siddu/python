...
| # | Video | Creator | Lang |
|---|-------|---------|------|
| 1 | [Metaclasses & How Classes Really Work](https://www.youtube.com/watch?v=NAQEj-c2CI8) | Tech With Tim | 🇬🇧 |
...
print(type(User))      # <class 'type'> ← User is an instance of 'type'!
print(type(type))      # <class 'type'> ← type is its own metaclass!

# Object hierarchy:
# type → creates → User (class) → creates → User() (instance)
...
# class BadSerializer(Serializer):    # ❌ TypeError!
#     def serialize(self): pass
#     # Missing deserialize!

class GoodSerializer(Serializer):     # ✅ OK
    def serialize(self): pass
    def deserialize(self): pass
```

> 💡 **In practice**: Most developers never need to write metaclasses. Use `ABC` for interfaces, `__init_subclass__` for class registration. Metaclasses are for framework authors.
...
class MyClass:
    def __new__(cls, *args, **kwargs):
        """Called FIRST — creates the instance."""
...
p = Product("Laptop", 75000, 10)    # ✅ OK
# Product("", 100, 5)               # ❌ ValueError: Invalid value for name
# Product("Phone", -500, 5)         # ❌ ValueError: Invalid value for price
```
...
Introspect live objects — great for debugging, documentation tools, and frameworks.
...
### `__setattr__` — Called on EVERY Assignment
```python
class TrackedObject:
...
### `__getattribute__` — Called on EVERY Access (Dangerous!)
```python
class LoggedAccess:
...
# eval(user_input)    # ❌ DELETES YOUR ENTIRE SYSTEM!!!

user_input = "__import__('subprocess').call(['cat', '/etc/passwd'])"
# eval(user_input)    # ❌ Reads password file!
...
# ast.literal_eval("__import__('os')")              # ❌ ValueError (blocked!)
...
result = actions["add"](3, 4)    # 7 — safe!
```

### When Are They Acceptable?
```python
# ✅ Template engines (Jinja2, Mako — they use exec safely)
# ✅ REPL/debugging tools (you control the input)
# ✅ Code generation tools (trusted source code)
# ❌ NEVER with user input
# ❌ NEVER in web applications
# ❌ NEVER in APIs
```
...
# ---- AVOID ----
# eval(), exec() → security risk! Use ast.literal_eval() or json.loads()
```

---

> 📝 **Next up**: 7.6 Type Hints & Static Analysis — typing, Pydantic, mypy, marshmallow
