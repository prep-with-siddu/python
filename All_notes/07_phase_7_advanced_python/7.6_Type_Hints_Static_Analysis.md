# 7.6 Type Hints & Static Analysis — Notes
...
| # | Video | Creator | Lang |
|---|-------|---------|------|
| 1 | [Python Typing — Type Hints & Annotations](https://www.youtube.com/watch?v=QORvB-_mbZ0) | Tech With Tim | 🇬🇧 |
...
def first(items: list[T]) -> T:
    """Return first item — type is preserved!"""
    return items[0]
...
int_stack.push(42)       # ✅
# int_stack.push("hi")   # ❌ mypy catches this!
...
Duck typing with type safety! Define what methods an object must have — no inheritance needed.
...
# render(NotDrawable())  # ❌ mypy error: doesn't have draw()
...
user: UserDict = {"name": "Sid", "age": 25, "email": "sid@test.com"}    # ✅
# user: UserDict = {"name": "Sid"}    # ❌ Missing fields
...
Add metadata to types — used heavily by Pydantic, FastAPI, and other frameworks.
...
# → Query parameter, must be between 1 and 100
```
...
result: int = greet("Sid")    # ❌ mypy: Incompatible types
                               # assigned "str" to "int"
...
print(user.age)           # 25 (int! — auto-converted from "25")
...
user2 = User.model_validate_json(json_str)   # → User object
```
...
# Product(name="", price=-100, quantity=-1)    # ❌ All fail validation!
```
...
## 9. Pydantic v2 — Validators, Model Config, Computed Fields
...
# user.name = "New"    # ❌ frozen=True makes it immutable!
# User(name="Sid", age=25, extra_field="x")  # ❌ extra="forbid"
```
...
| Speed | **Much faster** (Rust core) ✅ | Slower |
| FastAPI | Native integration ✅ | No |
| Flask | Works | Native ecosystem ✅ |
| JSON Schema | Auto-generated ✅ | Plugin needed |
| Recommendation | New projects ✅ | Flask legacy projects |
...
> 💡 `cattrs` is lightweight and fast. Use it when you need simple struct/unstruct without full Pydantic validation.
...
> 📝 **Next up**: 7.7 Networking & HTTP from Python — requests, httpx, aiohttp, tenacity
