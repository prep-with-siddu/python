...
| # | Video | Creator | Lang |
|---|-------|---------|------|
| 1 | [Unit Testing Your Code with the `unittest` Module](https://www.youtube.com/watch?v=6tNS--WetLI) | Corey Schafer | 🇬🇧 |
...
> 💡 `unittest` works but is verbose. **Use `pytest` for new projects** — it's simpler and more powerful.

---

## 2. `pytest` — Setup, Writing Tests, Assertions
...
|---------|--------|----------|
| Syntax | Plain `assert` ✅ | `self.assertEqual()` |
| Boilerplate | Minimal ✅ | Class required |
| Fixtures | Powerful, composable ✅ | setUp/tearDown only |
| Plugins | 1000+ plugins ✅ | Limited |
| Parametrize | Built-in ✅ | Needs third-party |
| Auto-discovery | ✅ | Basic |
| Recommendation | **New projects ✅** | Legacy/stdlib only |
...
def db_connection():
    ...
    yield conn              # ← Test runs here (between setup and teardown)
...
def reset_database(db_session):
    """Automatically runs before EVERY test — no need to pass it."""
    yield
    db_session.rollback()
```
...
Run the same test with multiple inputs — DRY testing!
...
Shared fixtures and hooks — pytest auto-discovers `conftest.py` files.

```
tests/
├── conftest.py              # Shared across ALL tests
├── unit/
│   ├── conftest.py          # Shared within unit/ only
│   ├── test_models.py
│   └── test_services.py
├── integration/
│   ├── conftest.py          # Shared within integration/ only
│   ├── test_api.py
│   └── test_database.py
└── e2e/
    ├── conftest.py
...
```python
# ✅ DO: Keep fixtures focused and composable
@pytest.fixture
def user_data():
...
# ❌ DON'T: Giant fixtures that do everything
# ❌ DON'T: Logic in conftest — keep it simple
# ❌ DON'T: Import conftest — pytest handles it automatically
```
...
### What to Cover
```
✅ Cover: Business logic, API endpoints, utilities, error paths
❌ Don't chase 100%: Config files, abstract methods, type hints
...
# Run: pytest → FAILS (no endpoint exists yet)
...
```
✅ Forces you to think about API design BEFORE coding
...
│   │   ├── test_models.py
│   │   └── test_services.py
│   ├── integration/         # Tests with DB, APIs
│   │   ├── test_api.py
│   │   └── test_database.py
...
> 📝 **Next up**: 8.2 Integration & API Testing — test clients, factory_boy, faker, locust
