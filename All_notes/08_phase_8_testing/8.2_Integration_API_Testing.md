# 8.2 Integration & API Testing — Notes
...
| 2 | [Python Requests Tutorial](https://www.youtube.com/watch?v=tb8gHvYlCFs) | Corey Schafer | 🇬🇧 |
...
✅ Failure rate
✅ Concurrent users handling capacity
```
...
Tests the **entire system** from user perspective — DB, APIs, external services, everything.

```python
# ---- E2E: Full user registration → login → action flow ----
...
### E2E Best Practices
```
✅ Test critical user flows (sign up, checkout, payment)
✅ Use real (test) database — not mocks
✅ Keep E2E tests few but meaningful
❌ Don't test every edge case in E2E — that's unit test job
❌ Don't mock external services in E2E (use test/sandbox environments)
```
...
Compare output against saved "snapshots" — great for API responses.
...
# Consumer defines what it expects → provider verifies it matches
...
✅ Frontend consuming your backend API
✅ Third-party integrations
✅ API versioning — ensure v2 doesn't break v1 consumers
```
...
def test_external_api():
    ...
    # Your code makes a real requests.get() call — but gets mock response!
...
```
Unit Tests (8.1)          → Mock everything, test logic
Integration Tests (8.2)   → Real DB, test client, mock externals
E2E Tests                 → Real everything, test flows
Load Tests (locust)       → Test performance under pressure
Contract Tests            → Verify API agreements
```

---

> 📝 **Phase 8 Complete!** Next: Phase 9 — Git, CI/CD & Code Quality
